...
-- Первый вариант сети (устаревшее)
...
-- Второй вариант сети (устаревшее)
...
function model.makeDeep300wlpCNN(mm)
    ...
    net:add(nn.SpatialConvolution(1, 4, 3, 3))
    net:add(nn.SpatialBatchNormalization(4))
    net:add(nn.ReLU(false))
    net:add(nn.SpatialMaxPooling(2, 2, 2, 2))

    net:add(nn.SpatialConvolution(4, 8, 3, 3))
    net:add(nn.SpatialBatchNormalization(8))
    net:add(nn.ReLU(false))
    net:add(nn.SpatialMaxPooling(2, 2, 2, 2))
